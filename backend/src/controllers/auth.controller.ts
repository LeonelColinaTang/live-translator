...
export const signup = async (req: Request, res: Response) => {
  try {
    const { username, email, password, confirmPassword, prefLang } = req.body;

    if (!username || !email || !password || !confirmPassword || !prefLang) {
    ...
    const avatar = `https://avatar.iran.liara.run/username?username=${username}&bold=true&length=2`
